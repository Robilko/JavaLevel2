                ...
                try {
                    while (!isAuthorized && goOn) { // цикл авторизации.
                        ...
                        if (message.startsWith("/authok")) { // если от сервера пришло сообщение,
                            ...
                            goOn = false;
                        } else if (message.equalsIgnoreCase("/error timeout")){// сюда мы попадаем если сервер нам написал что то умное чем authok и end
                            // можно было бы тут написать if startsWith(error)
                            callOnError.callback("Time for authorization is out");
                        } else {
